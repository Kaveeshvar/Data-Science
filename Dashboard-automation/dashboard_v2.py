...
# ── v3 ML Engine imports (graceful) ─────────────────────────────
try:
    from ml_engine.target_detection import detect_target
    from ml_engine.training import train_model
    from ml_engine.evaluation import evaluate_model
    from ml_engine.export import export_model, get_inference_metadata
    from ml_engine.segmentation import should_run_segmentation, run_segmentation
    from ml_engine.anomaly import run_anomaly_detection
    from ml_engine.html_sections import (
...
def _render_footer() -> str:
    return """
  <div class="footer">
    <span>Generated by Pro Dashboard Generator v3</span>
...
def render_html(
    *,
    ...
    quality: dict,
    narrative: list,
    # ── v3 optional sections ──
    v3_sections: Optional[List[str]] = None,
) -> str:
    css = generate_theme_css(theme_id)
    # Add v3 CSS if available
    v3_css = ""
    if _V3_AVAILABLE:
        try:
            v3_css = get_v3_css()
    ...
    # ── Inject v3 sections after ML recommendation & insights ──
    if v3_sections:
        sections.extend(v3_sections)
...
def main(
    ...
    # ── 7. v3 ML Engine sections ───────────────────────────────
    v3_sections: List[str] = []
    if _V3_AVAILABLE and not disable_ml:
        try:
            v3_sections = _run_v3_ml_pipeline(df, roles, theme_id, out_html)
        except Exception as e:
            log.warning("v3 ML pipeline failed (non-fatal): %s", e)
...
def _run_v3_ml_pipeline(
    ...
    """
    Run v3 ML pipeline: target detection → training → evaluation → export →
    segmentation → anomaly → HTML sections.  Never crashes the main pipeline.
    """
    sections: List[str] = []
    output_dir = Path(out_html).parent
    numeric_cols = roles.get("numeric", [])

    # ── 7a. Global filter panel ─────────────────────────────
    try:
        cat_cols = roles.get("categorical", [])
        filter_html = render_global_filter_panel(df, cat_cols, theme_id)
        if filter_html:
            sections.append(filter_html)
    except Exception as e:
        log.warning("Filter panel failed: %s", e)
    ...
    log.info("Target detection: col=%s, type=%s, confidence=%.2f",
             target_col, problem_type, confidence)

    # ── 7c. Auto training + evaluation + export ─────────────
    train_result = None
    eval_result = None
    if target_col and problem_type and confidence >= 0.15:
        try:
            log.info("Training %s model on target '%s'...", problem_type, target_col)
            train_result = train_model(df, target_col, problem_type)
            log.info("Model trained: %s (CV: %.4f)",
                     train_result["model_type"],
                     train_result["metrics"].get("cv_score", 0))

            # Evaluation
            eval_result = evaluate_model(train_result, theme_id)
            perf_html = render_model_performance_section(eval_result, train_result, theme_id)
            sections.append(perf_html)

            # Feature importance
            fi_html = render_feature_importance_section(train_result, theme_id)
            if fi_html:
                sections.append(fi_html)

            # Export artefacts
            try:
                export_paths = export_model(train_result, eval_result, output_dir)
                log.info("Exported artefacts: %s",
                         {k: ("✓" if v else "✗") for k, v in export_paths.items()})
            except Exception as e:
                log.warning("Model export failed: %s", e)
                export_paths = {}

            # Prediction simulator
            try:
                import base64
                inference_meta = get_inference_metadata(train_result, df)
                onnx_ok = bool(export_paths.get("onnx"))
                
                # Embed ONNX model as base64 for in-browser inference
                onnx_model_b64 = None
                if onnx_ok and export_paths.get("onnx"):
                    try:
                        with open(export_paths["onnx"], "rb") as f:
                            onnx_b64_bytes = base64.b64encode(f.read()).decode("utf-8")
                            onnx_model_b64 = onnx_b64_bytes
                        log.info("✓ Embedded ONNX model as base64 (%d KB)", len(onnx_model_b64) // 1024)
                    except Exception as e:
                        log.warning("Failed to embed ONNX model: %s (predictions via model.pkl instead)", e)
                        onnx_ok = False
                
                if not onnx_ok:
                    pkl_path = export_paths.get("model_pkl")
                    if pkl_path:
                        log.info("Using model.pkl for predictions (%s)", pkl_path)
                
                sim_html = render_prediction_simulator(inference_meta, onnx_ok, theme_id, onnx_model_b64)
                if sim_html:
                    sections.append(sim_html)
            except Exception as e:
                log.warning("Prediction simulator failed: %s", e)

        except Exception as e:
            log.warning("Model training/evaluation failed: %s", e)
    else:
        log.info("Skipping model training (no confident target detected).")

    # ── 7d. Segmentation ────────────────────────────────────
    try:
        target_detected = target_col is not None and confidence >= 0.15
        if should_run_segmentation(df, target_detected, numeric_cols):
            log.info("Running segmentation...")
            seg_result = run_segmentation(df, numeric_cols, theme_id)
            if seg_result:
                seg_html = render_segmentation_section(seg_result, theme_id)
                sections.append(seg_html)
                log.info("Segmentation: %d clusters (sil=%.3f)",
                         seg_result["n_clusters"], seg_result["silhouette"])
    except Exception as e:
        log.warning("Segmentation failed: %s", e)

    # ── 7e. Anomaly detection (IsolationForest) ─────────────
    try:
        if len(numeric_cols) >= 2:
            log.info("Running IsolationForest anomaly detection...")
            anom_result = run_anomaly_detection(df, numeric_cols, theme_id)
            if anom_result:
                anom_html = render_anomaly_section(anom_result, theme_id)
                sections.append(anom_html)
                log.info("Anomalies: %d (%.1f%%)",
                         anom_result["n_anomalies"], anom_result["pct_anomalies"])
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Pro Dashboard Generator v3")
    ...
    parser.add_argument("--disable-ml", action="store_true",
                        help="Disable v3 ML pipeline (target detection, training, segmentation, anomaly)")
